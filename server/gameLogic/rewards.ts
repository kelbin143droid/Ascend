...
export function calculateHabitXP(habit: Habit): number {
  const streakMultiplier = Math.min(STREAK_MULTIPLIER_CAP, 1 + habit.currentStreak * 0.05);
  const difficultyMultiplier = 1 + (habit.difficultyLevel - 1) * 0.2;
  return Math.floor(BASE_HABIT_XP * streakMultiplier * difficultyMultiplier);
}
...
const BADGE_DEFINITIONS: BadgeDefinition[] = [
  ...
  { type: "consistency_king", name: "Consistency King", description: "Reached maximum momentum on any habit" },
];
  ...
  for (const def of BADGE_DEFINITIONS) {
    ...
    switch (def.type) {
      ...
      case "consistency_king":
        if (habit.momentum >= 0.95) newBadges.push(def);
        break;
    }
  }
