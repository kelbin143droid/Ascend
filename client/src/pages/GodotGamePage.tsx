...
// DB slot name → Godot slot name (Helmet / Boots not tracked in DB yet — omitted)
...
export default function GodotGamePage() {
  ...
  // Forward ascend:stats-updated events (from HunterProfilePage allocation)
  useEffect(() => {
  ...
  // ── Equipment: send on ready + whenever player inventory/equipment changes ──
  const sendEquipment = useCallback(() => {
    const iframe = iframeRef.current;
    if (!iframe?.contentWindow || !readyRef.current || !player) return;
    const equipment = buildEquipment(player);
    iframe.contentWindow.postMessage({ type: "SET_EQUIPMENT", equipment }, "*");
  }, [player]);

  useEffect(() => {
    if (readyRef.current) sendEquipment();
  }, [sendEquipment]);

  // Forward ascend:equipment-updated events (for immediate dispatch before cache refresh)
  useEffect(() => {
    const handler = (e: Event) => {
