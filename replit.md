...
-   **Workout Builder System**: A 4-level workout progression system (Entry/Beginner/Intermediate/Advanced) on the Training page alongside the existing Daily Flow tab. Defined in `client/src/lib/workoutPlans.ts` (exercise data + `buildWorkoutActivity()` converter to `ActivityDefinition`). Includes automatic warm-up (3 exercises), an integrated Cardio Module (Off/Light/Moderate/Intense, before or after workout). Uses the existing `GuidedActivityEngine` to run sessions.
-   **Hybrid Progression Engine**: Post-workout scoring system in `client/src/lib/workoutProgressionEngine.ts` (pure, side-effect-free, modular for future AI replacement). Score = completion(+20) + sets(+40) + reps(+30) + difficulty modifier (Easy+10, Hard-15). Rolling 3-session average drives zone decisions: progress(≥80) → micro-progression in order: reps→sets→level_up; maintain(60-79); regress(<60 or 2×hard) → reps→sets→level_down. Micro-progression state (repsBonus, setsBonus per level) stored in `localStorage` via `workoutProgressStore.ts`. After each workout, a bottom-sheet modal asks difficulty, then shows a score breakdown card with recommendation. Level-up/down can be applied with one tap.
-   **Breathing Progression System**: 4-phase scalable breathing skill tracked in `client/src/lib/breathingProgressionSystem.ts` (pure engine) + `client/src/lib/breathingStore.ts` (localStorage). Phase 1 = 4-4-6 (2 min), Phase 2 = 4-4-8 (3 min), Phase 3 = 5-5-8 (4 min), Phase 4 = Adaptive (5-8 min dynamic). After each Calm Breathing session, `BreathingFeedbackModal.tsx` captures 3 questions (difficulty / rhythm / mind-wandering), computes a 0-100 score, applies adaptive micro-adjustments (exhale +1 or duration +30 when score ≥ 85; hold -1 or duration -30 when score < 40), checks phase unlock conditions (min session count + 80% positive feedback), awards base/streak/perfect XP, and updates the store. `activityEngine.ts` reads the profile to inject the current pattern and duration into the breathing step (replacing hardcoded 4-4-6). The `DailyFlowEngine` intercepts `phase1_meditation` completion to show `BreathingFeedbackModal` immediately (before advancing to agility), then continues the flow. Phase badge shown on the pre-session card. End-of-flow feedback modal now handles strength only.
