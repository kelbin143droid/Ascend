...
-   **AI Coach**: A rule-based coaching engine with a defined personality system. Behaves as a hybrid system guide, calm RPG narrator, and behavioral psychology coach. Tone: calm, observant, non-judgmental, concise, supportive without hype. Never uses guilt language (banned phrases enforced via `sanitizeCoachText()`). All responses follow Acknowledge → Reframe → Suggest pattern. Integrates with Language Evolution System (`applyLanguageStage`) and behavioral anchors (cluster detection via `detectAnchorClusters()`). Coach references anchor patterns when contextually relevant (e.g., "Your resets tend to happen in the evening"). Located in `server/gameLogic/aiCoach.ts`.
...
-   **Sectograph** (`/sectograph`): Unified time system page with 3 tabbed views — Timeline (radial Sectograph clock, default), Calendar (monthly event management), and Week (weekly overview). Replaces standalone Calendar sidebar item. Locked before Day 7 with message "Sectograph unlocks once your rhythm begins." **Temporarily unlocked (unlockDay=1) for testing.**
    ...
    -   **Behavioral Anchoring**: Onboarding completion timestamps rendered as amber "Reset" markers on the Sectograph outer ring. API endpoint `GET /api/player/:id/behavioral-anchors` returns guided session completions with hour/minute. Markers section shows last 5 anchors. `BehavioralAnchor` interface exported from `Sectograph.tsx`.
    -   **Introduction Overlay**: First-visit overlay on Sectograph page explaining the time system, color codes (green=present, amber=resets, purple=focus). Stored in `ascend_sectograph_intro_seen` localStorage flag. Dismissed via "Begin Observing" button.
    -   **Focus Sessions**: Configurable duration (5/10/15/25/30/45/60 min), launchable from Timeline tab or by clicking free windows. Appear as purple blocks on Sectograph SVG. Active session shows countdown timer with pause/cancel controls. API endpoint `POST /api/player/:id/start-focus-session`. `ActiveFocusBlock` interface exported from `Sectograph.tsx`.
