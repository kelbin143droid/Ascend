...
-   **Phase 1 Activities**: Strength Micro Circuit (instruction → push-ups → cardio jog → crunches → rest, ~2 min), Agility Mobility Flow (instruction → neck CW/CCW → shoulder rolls → torso twist → forward fold, ~60-90s), Calm Breathing (4-2-6 pattern, 2 min), Vitality Check (hydration + sleep).
-   **Adaptive Training Scaling**: 5 difficulty tiers per category. Phase 1 caps at Tier 3. Progression: 3 completions in a row → tier up. Regression: 3 missed days → tier down. XP multipliers: T1=1.0, T2=1.1, T3=1.2, T4=1.3, T5=1.5. Strength tiers define pushups (5→10), cardio (30s→60s), crunches (10→20). Tracked per category: tier, completionStreak, missedDays, sessionsCompleted, lastSessionDate. Server: `server/gameLogic/trainingScaling.ts`. API: `GET /api/player/:id/training-scaling`. Schema field: `trainingScaling` (JSONB on players table).
